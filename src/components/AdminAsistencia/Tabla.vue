<template>
  <div>
    <div class="text-xs-center elevation-6 py-3">
        Horas Netas Seleccionadas: <strong>{{mstoonlyhours(horasTotalesSelected)}}</strong>
    </div>
    <v-card>
...
export default {
    ...
    watch: {
        async userselector(val,oldval){
            await this.$store.dispatch('usersasistencia/closeDBChannel', {clearModule: true})
            .then(async ()=>{
                await this.$store.dispatch('usersasistencia/openDBChannel', {idusuario: val}).catch(console.error)
                })
            .catch(console.error)
            //this.$store.dispatch('usersasistencia/openDBChannel', {idusuario: val}).catch(console.error)
        },
        dialog (val) {
            val || this.close()
        }
